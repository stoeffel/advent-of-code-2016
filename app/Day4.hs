...
rooms :: Parsec String st Rooms
rooms = room `endBy` newline <* eof
  where
    room = Room <$> roomName <*> sectorId <*> checkSum
    roomName = init <$> (many1 $ letter <|> dash)
...
:}
Right [Room {roomName = "hqcfqwydw-fbqijys-whqii-huiuqhsx", sectorId = 660, checkSum = "qhiwf"},Room {roomName = "oxjmxdfkd-pzxsbkdbo-erkq-ixyloxqlov", sectorId = 913, checkSum = "xodkb"},Room {roomName = "bpvctixr-eaphixr-vgphh-gthtpgrw", sectorId = 947, checkSum = "smrkl"},Room {roomName = "iwcjapey-lhwopey-cnwoo-wymqeoepekj", sectorId = 992, checkSum = "eowpy"}]
-}
parseRooms :: String -> Either ParseError Rooms
parseRooms = parse rooms "Invalid Rooms"


-- |
-- >>> valid (Room "aaaadddbbcaaa" 123 "adbc")
-- True
-- >>> valid (Room "x-xxxxxassdffffeeyyyyyyymmmmmm" 123 "ymxfe")
-- True
-- >>> valid (Room "x-xxxxxassdffffeeyyyyyyymmmmmm" 123 "ymafe")
-- False
-- >>> valid (Room "iruzfrtkzmv-dzczkrip-xiruv-treup-tfrkzex-fgvirkzfej" 633 "rzfik")
-- True
valid :: Room -> Bool
valid (Room roomName _ checkSum) =
  (==) checkSum
  $ map snd
  $ take 5
  $ sortBy byLengthAndChar
  $ map (length &&& head)
  $ group
  $ sort
  $ filter ((/=) '-') roomName
    where
      byLengthAndChar (l1, c1) (l2, c2) =
        if l1 == l2 then
          compare c1 c2
        else
          compare l2 l1
...
-- |
-- >>> countValidRooms "asb-123[fff]\nxxxxxxass-dff-ffeeyyyyyyymmmmmm-123[ymxfe]\nhqcfqwydw-fbqijys-whqii-huiuqhsx-660[qhiwf]\n"
-- Right 783
-- >>> countValidRooms input
-- Right 245102
countValidRooms :: String -> Either ParseError Int
countValidRooms str =
  sum . map sectorId . filter valid <$> parseRooms str
...
shiftStr n = map (shiftChr (rem n 26))


-- |
-- >>> findRooms "north" input
-- Right [Room {roomName = "northpole-object-storage", sectorId = 324, checkSum = "chsfb"}]
findRooms :: String -> String -> Either ParseError Rooms
findRooms x str =
  filter (isInfixOf x . roomName)
  . map decriptRoom
  . filter valid
  <$> parseRooms str
  where
    decriptRoom r@(Room roomName sectorId _) =
      r { roomName = shiftStr sectorId roomName }
